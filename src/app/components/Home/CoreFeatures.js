...
import { Camera, Brain, MessageSquare, BookOpen, Sparkles, BarChart3, ClipboardList, Bot } from "lucide-react";
...
export default function CoreFeatures() {
  const features = [
    {
      ...
      icon: <Camera className="w-8 h-8 text-[var(--color-primary)]" />,
    },
    {
  ...
  // Duplicate list for infinite looping
      ...
      <div className="overflow-hidden w-full scroll-wrapper">
        <div className="scroll-container gap-6 py-4">
          {infiniteList.map((feature, index) => (
            <div
              key={index}
              className="min-w-[280px] max-w-[280px] rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 flex flex-col items-start justify-start text-left hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
            >
              <div
                className={`p-3 rounded-xl mb-4 flex items-center justify-center bg-[var(--color-primary)]/10`}
              >
